...
category: logistics
api_name: set_pause_status
url: "https://open.shopee.com/documents/v2/v2.logistics.set_pause_status?module=95&type=1"
...
**Category:** LOGISTICS
**API:** set_pause_status

**Method:** POST
**HTTP Path:** /api/v2/logistics/set_pause_status
...
| --- | --- | --- |
| Live | Global | https://partner.shopeemobile.com/api/v2/logistics/set_pause_status |
| Live | Chinese Mainland | https://openplatform.shopee.cn/api/v2/logistics/set_pause_status |
| Live | Brazil | https://openplatform.shopee.com.br/api/v2/logistics/set_pause_status |
| Sandbox | Global | https://openplatform.sandbox.test-stable.shopee.sg/api/v2/logistics/set_pause_status |
| Sandbox | Chinese Mainland | https://openplatform.sandbox.test-stable.shopee.cn/api/v2/logistics/set_pause_status |
...
| --- | --- | --- | --- | --- |
| is_paused | boolean | True |  | The target pause status that seller wants to update to. Applicable values: - true: Trigger pause. All relevant channels will be paused and will not have any new incoming orders (fulfillment of existing orders will not be affected). Meanwhile, the system will start deducting the daily pause quota and automatically calculate the pause end time based on the remaining quota. - false: Trigger manual resume. No channels are paused and may have new incoming orders. The remaining daily quota will stop being consumed and be retained until reset the next day. Note: Due to the system cache synchronization mechanism, there may be an approximately 15-second delay before the pause/resume operation takes effect. It is recommended to call the v2.logistics.get_pause_status for confirmation after the update. |
...
HttpResponse<String> response = Unirest.post("https://partner.shopeemobile.com/api/v2/logistics/set_pause_status?access_token=access_token&partner_id=partner_id&shop_id=shop_id&sign=sign&timestamp=timestamp")
...
curl_setopt_array($curl, array(
  CURLOPT_URL => 'https://partner.shopeemobile.com/api/v2/logistics/set_pause_status?access_token=access_token&partner_id=partner_id&shop_id=shop_id&sign=sign&timestamp=timestamp',
...
```bash
curl --location --request POST 'https://partner.shopeemobile.com/api/v2/logistics/set_pause_status?access_token=access_token&partner_id=partner_id&shop_id=shop_id&sign=sign&timestamp=timestamp' \
--header 'Content-Type: application/json' \
--data-raw '{
   "is_paused": true
}'
...
import requests
import json
url = "https://partner.shopeemobile.com/api/v2/logistics/set_pause_status?access_token=access_token&partner_id=partner_id&shop_id=shop_id&sign=sign&timestamp=timestamp"
payload=json.dumps({
  "is_paused": True
})
headers = {
...
| --- | --- | --- |
| logistics.no_remaining_pause_quota | No remaining pause quota left for the shop |  |
| logistics.shop_not_in_feature_toggle | Shop is not whitelisted for the pause feature |  |
...
```json
{
    "error": "logistics.shop_not_in_feature_toggle",
    "message": "Shop is not whitelisted for the pause feature.",
    "request_id": "c955f5939580785debf7aaec62ec8d07"
}
```
