...
category: logistics
api_name: mass_ship_order
url: "https://open.shopee.com/documents/v2/v2.logistics.mass_ship_order?module=95&type=1"
...
**Method:** POST
**HTTP Path:** /api/v2/logistics/mass_ship_order
...
| non_integrated | object | False |  | Optional parameter when get_mass_shipping_parameter returns "non-integrated" under "info_needed". |
| tracking_number | object[] | False |  | Optional parameter for non-integrated channel order. The tracking number assigned by the shipping carrier for item shipment. |
| package_number | string | True |  | Shopee's unique identifier for the package under an order. |
| tracking_number | string | True |  | Optional parameter for non-integrated channel order. The tracking number assigned by the shipping carrier for item shipment. |
