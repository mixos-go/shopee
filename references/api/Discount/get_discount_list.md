...
url: "https://open.shopee.com/documents/v2/v2.discount.get_discount_list?module=99&type=1"
...
**Category:** DISCOUNT
**API:** get_discount_list

**Method:** GET
**HTTP Path:** /api/v2/discount/get_discount_list
...
| --- | --- | --- |
| Live | Global | https://partner.shopeemobile.com/api/v2/discount/get_discount_list |
| Live | Chinese Mainland | https://openplatform.shopee.cn/api/v2/discount/get_discount_list |
| Live | Brazil | https://openplatform.shopee.com.br/api/v2/discount/get_discount_list |
| Sandbox | Global | https://openplatform.sandbox.test-stable.shopee.sg/api/v2/discount/get_discount_list |
| Sandbox | Chinese Mainland | https://openplatform.sandbox.test-stable.shopee.cn/api/v2/discount/get_discount_list |
...
| --- | --- | --- | --- | --- |
| discount_status | string | True | ongoing | The status filter for retriveing discount list. Available value: upcoming/ongoing/expired/all. |
| page_no | int | True | 1 | Specifies the page number of data to return in the current call. Starting from 1. if data is more than one page, the page_no can be some entry to start next call. |
| page_size | int | True | 100 | If many items are available to retrieve, you may need to call GetDiscountsList multiple times to retrieve all the data. Each result set is returned as a page of entries. Use the Pagination filters to control the maximum number of entries (<= 100) to retrieve per page (i.e., per call), the offset number to start next call. This integer value is used to specify the maximum number of entries to return in a single "page" of data. |
| update_time_from | timestamp | False | 1643860467 | The update_time_from and update_time_to fields specify a date range for retrieving orders (based on the discount update time). The maximum date range that may be specified with the update_time_from and update_time_to fields is 30 days. |
| update_time_to | timestamp | False | 1646020467 | The update_time_from and update_time_to fields specify a date range for retrieving orders (based on the discount update time). The maximum date range that may be specified with the update_time_from and update_time_to fields is 30 days. |
