...
category: returns
api_name: cancel_dispute
url: "https://open.shopee.com/documents/v2/v2.returns.cancel_dispute?module=102&type=1"
...
**Method:** POST
**HTTP Path:** /api/v2/returns/cancel_dispute
...
| --- | --- | --- |
| Live | Global | https://partner.shopeemobile.com/api/v2/returns/cancel_dispute |
| Live | Chinese Mainland | https://openplatform.shopee.cn/api/v2/returns/cancel_dispute |
| Live | Brazil | https://openplatform.shopee.com.br/api/v2/returns/cancel_dispute |
| Sandbox | Global | https://openplatform.sandbox.test-stable.shopee.sg/api/v2/returns/cancel_dispute |
| Sandbox | Chinese Mainland | https://openplatform.sandbox.test-stable.shopee.cn/api/v2/returns/cancel_dispute |
...
| --- | --- | --- | --- | --- |
| return_sn | string | True | 2412100F5217W3H | Shopee's unique serial number identifier for a Return Refund request. Note: Sellers can only cancel compensation disputes, not normal disputes. This means that sellers can only cancel disputes when the return_status is ACCEPTED and the compensation_status is COMPENSATION_REQUESTED . |
| email | string | True | test@example.com | The operator's email address. For operation record keeping purposes (same as v2.returns.dispute API). |
...
```bash
curl --location --request POST 'https://partner.shopeemobile.com/api/v2/returns/cancel_dispute?access_token=access_token&partner_id=partner_id&shop_id=shop_id&sign=sign&timestamp=timestamp' \
--header 'Content-Type: application/json' \
--data-raw '{
   "email": "test@example.com",
   "return_sn": "2412100F5217W3H"
